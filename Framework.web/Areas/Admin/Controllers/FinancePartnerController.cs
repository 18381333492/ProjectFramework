...
namespace Framework.web.Areas.Admin.Controllers
{
    ...
    /// 合伙人财务控制器
    /// </summary>
    public class FinancePartnerController : SuperController
    {
        #region 视图
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.iPartnerAviliableMoney = LoadInterface<IFinanceManager>().GetPartnerAviliableMoney(SessionUser.User.ID);
            return View();
        }
        ...
        public ActionResult Apply()
        {
            ...
            ViewBag.iMinimumMoney = manager.GetWithdrawCash("合伙人")["iMinimumMoney"];//最低提现额度
            ViewBag.iPartnerAviliableMoney = LoadInterface<IFinanceManager>().GetPartnerAviliableMoney(SessionUser.User.ID)["iprice"];
        ...
        {
            IFinanceManager manager = LoadInterface<IFinanceManager>();
            Dictionary<string,object> dic=manager.GetWithdrawCash("合伙人");
            //最新的提现记录
            EHECD_WithdrawCashDTO dto = manager.GetApplierInfo(SessionUser.User.ID);
        ...
        public void GetDetailPartnerList()
        {
            ...
            var tAviliableMoney = LoadInterface<IFinanceManager>().GetPartnerAviliableMoneyByTT(SessionUser.User.ID);
            IFinanceManager manager = LoadInterface<IFinanceManager>();
            Dapper.PagingRet<Dictionary<string, object>> list = manager.GetFinancePartnerList(pageInfo, RequestParameters.dynamicData.data,SessionUser.User.ID.Value);
           ...
           // --1提现，2充值，3退款，4购买 5收入佣金,6服务费,7订单收入,8.佣金支出9.服务费支出,10合伙人提成
            ...
            totalPrices = totalPrices - returnPrices;//实际收入的价格
            ...
            result.Data = result.Data = new Dictionary<string, object>() {
                { "list",list},
                { "tAviliableMoney",tAviliableMoney},
                { "totalPrices",totalPrices},
                { "PartnerPrices",setting.iPartnerCommissionPrecent.Value.ToDecimal()*0.01.ToDecimal()*totalServer}
            };
        ...
        public void SendMessage()
        {
            /*要获取短信验证码的登录名*/
            var param = SessionUser.User.sLoginName;
            var timeoutparam = SessionUser.User.sLoginName + "tttt";
